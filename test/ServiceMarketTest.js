...
describe("ServicesMarket", ()=>{
    ...
    beforeEach(async()=>{
        ...
        item = await ethers.getContractFactory("ServicePurchase");
        service = await item.deploy(market.address);
        await service.deployed();

        //create tokens
        await service.connect(seller).createToken("0");
        await service.connect(seller).createToken("1");
        await service.connect(seller).createToken("2");
        
    });

    describe("createService", () => {
        it("Should create new service", async() => {
            expect(await market.connect(seller).createService(service.address, 1, 30, {value: 1000})).to.emit(market, "ServiceCreated");
        });
    });

    describe("abortService", ()=>{
        it("Should allow the seller to abort after creation", async()=>{
            await market.connect(seller).createService(service.address, 1, 30, {value: 1000});
    ...
    describe("confirmPurchaseConclusion", ()=>{
        it("Should allow the buyer confirm service conclusion", async() => {
            ...
            expect(await market.connect(buyer).confirmServiceConclusion(1)).
    ...
    describe("refundSeller", ()=>{
        it("Should allow the seller to withdraw initial value of service after contract conclusion", async ()=> {
            await market.connect(seller).createService(service.address, 1, 30, {value: 1000});
            await market.connect(buyer).purchaseService(1, {value: 2000});
            await market.connect(buyer).confirmServiceConclusion(1);
    ...
    describe("fetchAvailableServices", () => {
        it("Should query all available services", async ()=> {
            await market.connect(seller).createService(service.address, 1, 30, {value: 1000});
            await market.connect(seller).createService(service.address, 2, 30, {value: 1000});
            await market.connect(seller).createService(service.address, 3, 30, {value: 1000});
            let services = await market.fetchAvailableServices();
            services = await Promise.all(services.map(async i => {
                const tokenUri = await service.tokenURI(i.id);
                let listing = {
